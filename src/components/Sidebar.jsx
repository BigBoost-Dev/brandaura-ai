...
const Icons = {
  ...
  lightbulb: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18h6m-5 4h4M12 2a7 7 0 00-4 12.73V17a1 1 0 001 1h6a1 1 0 001-1v-2.27A7 7 0 0012 2z"/></svg>,
...
export default function Sidebar({ activeTab, onTabChange, resultsCount = 0 }) {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <nav className="p-1.5 space-y-0.5 overflow-y-auto" style={{ height: 'calc(100vh - 56px)' }}>
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center rounded-lg transition-all duration-200 ${
                expanded ? 'px-3 py-2.5 gap-3' : 'p-2.5 justify-center'
              } ${
                isActive 
            ...
            >
              ...
              <span className={`text-[13px] whitespace-nowrap transition-all duration-200 ${
                expanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
              }`}>
                {item.label}
              </span>
              {item.id === 'results' && resultsCount > 0 && expanded && (
                <span className="ml-auto text-[11px] px-1.5 py-0.5 rounded bg-white/[0.06] text-white/40">
                  {resultsCount}
                </span>
              )}
            </button>
